...
import { MenuDocument, MenuItemDecument, MenuItemModel, MenuItemSchema, MenuModel, MenuSchema } from "../models/MenuModel";
import RestaurantSchema, { RestaurantDoc, RestaurantModel } from "../models/RestaurantModel";
...
export default class RestaurantRepository implements IRestaurantRepository {
    ...
    async findByLocation(location: Location, page: number, pageSize: number
    ...
    async getMenus(restaurantId: string): Promise<Menu[]> {
        ...
        const menuItemModel = this.client.model<MenuItemDecument>(
            'MenuItem',
            MenuItemSchema
        ) as MenuItemModel
    ...
    private restaurantsFromPageResults(pageResults: PaginateResult<RestaurantDoc> | null) {
