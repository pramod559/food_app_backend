...
export default class RestaurantController {
    ...
    public async findByLocation(req: express.Request, res: express.Response) {
        ...
        try {
            ...
            const locaciton = new Location(parseFloat(longitude), parseFloat(latitude))

            return this.repository
                .findByLocation(locaciton, parseInt(page), parseInt(limit))
                .then((pageable) =>
                    res.status(200).json({
                        metadata: {
                            page: pageable.page,
    ...
    public async search(req: express.Request, res: express.Response) {
        try {
            const { page, limit, express.query } = req.query as {
                page: any
                limit: any
                query: string
            }
            return this.repository
                ...
                .catch((err: Error) => res.status(404).json({
                    error: err
                }))
